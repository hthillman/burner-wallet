...
import connext from "./connext.jpg";
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (!mnemonic) {
      mnemonic = this.generateViaMnemonic();
      localStorage.setItem("mnemonic", mnemonic);
    }
    const newPrivateKey = pkutils.getPrivateKeyFromMnemonic(mnemonic);
    this.setState({ newPrivateKey: newPrivateKey.replace("0x", "")});
    console.log(`SETTING META PRIVKEY: ${newPrivateKey.replace("0x", "")}`)
    localStorage.setItem("metaPrivateKey", newPrivateKey);
    console.log(`DONE SETTING META PRIVKEY`)
...
*/
  ...
  render() {
    ...
    console.log("FOUND Private Key: ", this.state.newPrivateKey)
                            ...
                            <SendWithLink
                              ...
                              sendWithLink={(amount, cb) => {
                                ...
                                const tokenAddress =
                                  ERC20TOKEN === false
                                    ? 0
                                    : this.state.contracts[ERC20TOKEN]._address;
                                // -- Temp hacks
